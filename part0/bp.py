...
def diy_adam(grad,weight):
    ...
    return weight
x = np.array([1,2,3,4]) #x.shape  (4,) 表示一维数组，数组中有4个元素
y = np.array([3,2,4,5])
...
numpy_model_w = copy.deepcopy(torch_model_w)
...
#pytorch的反向传播操作
torch_loss.backward()
print(torch_model.layer.weight.grad,"torch计算梯度")#查看某层权重的梯度
#手动实现反向传播
