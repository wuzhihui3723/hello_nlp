...
class TorchModel(nn.Module):
    def __init__(self,input_dim,sentence_length,vocab):
        ...
        self.pool = nn.MaxPool1d(sentence_length)  #该任务有一些字符很关键，取平均不太合理，max更合理
        # self.pool = nn.AvgPool1d(sentence_length)
        self.classify = nn.Linear(input_dim,3)
        # self.activation = torch.sigmoid
        self.activation = torch.relu   #在该任务中relu比sigmoid效果好
        self.dropout = nn.Dropout(0.1)
        self.loss = nn.functional.cross_entropy  #多分类常采用交叉熵作为损失函数，回归多用mse
    ...
    def forward(self,x,y=None):
        ...
        x = self.dropout(x) #input shape:(batch_size, sen_len, input_dim) (10,6,20)
        x = self.activation(x) #input shape:(batch_size, sen_len, input_dim) (10,6,20)
        x = self.pool(x.transpose(1,2)).squeeze() #input shape:(batch_size, sen_len, input_dim) (10,6,20)
...
def build_dataset(sample_length,vocab,sentence_length):
    dataset_x = []
    dataset_y = []
    for i in range(sample_length):#sample_length:每次训练的样本个数
